...
int main(int argc, char **argv)
{
	...
	if (argc != 2) {
		printf("Error: wrong number of arguments! | Usage: tsp_convert_lower input.txt\n");
		return EXIT_FAILURE;
	}
	input_file = fopen(argv[1], "r");
        if (input_file == NULL) {
		printf("Error opening file | Usage: tsp_convert_lower input.txt\n");
		return EXIT_FAILURE;
	}
	if(fscanf(input_file,"%d",&amount_of_cities) == EOF) {
		printf("Incorrect input in file %s | Usage: tsp_convert_lower input.txt\n",argv[1]);
		fclose(input_file);
		return EXIT_FAILURE;
	}
